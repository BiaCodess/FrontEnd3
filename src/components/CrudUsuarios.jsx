...
export default function CrudUsuarios() {
    ...
    const url = "https://back-end-api-alpha.vercel.app/usuarios/";//meu banco dados
    useEffect(() => { //causa um efeito na pag
        fetch(url)
            .then((respFetch) => respFetch.json())
            .then((respJson) => setUsuarios(respJson))//respJson recebe um usuario
    ...
    function apagarDados(cod) {
        axios.delete(url + cod)
            .then(() => setUsuarios(usuarios.filter(item =>
                item.id !== cod)))
            .catch((erro) => alert("Ocorreu um error!", erro));
    }
    ...
    function atualizaListaComNovoUsuario(response) {
        ...
        let users = usuarios;
        users.push(obj);//copia para pode att dps
        setUsuarios(users);//att 
        limparDados("");
    }
